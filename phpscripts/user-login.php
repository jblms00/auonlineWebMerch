...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $get_users_query = "SELECT * FROM users_accounts WHERE user_email = '$user_email' AND account_status = 'active'";
    ...
    if ($get_users_result && mysqli_num_rows($get_users_result) <= 0) {
    ...
    } else if ($fetch_users['account_status'] != 'active') {
        $data['status'] = "error";
        $data['message'] = "Your account is inactive. Please contact the admin.";
        ...
        $data['message'] = "Logging you in. Please hold on while we complete the process.";
        $data['status'] = "success";
    }
}
